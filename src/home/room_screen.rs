        ...
        if let Event::Actions(actions) = event {
            ...
            for action in actions {
                ...
                #[cfg(feature = "a2app")]
                {
                    ...
                    {
                        self.apply_mini_app_room_action(cx, room_action, &user_profile_sliding_pane);
                        continue;
                    }
                }
...
impl RoomScreen {
    ...
    fn apply_mini_app_room_action(
        ...
        match action {
            ...
            // This room may have just been shown, so its timeline may not be drawn yet.
            RoomAction::JumpToEvent(event_id) => {
                self.jump_to_event_after_draw(cx, event_id, String::from("the message a mini-app pointed at"));
            }
    ...
    pub fn set_displayed_room(
        ...
        if self.tl_state.is_some()
            ...
            && let Some(room_action) = crate::a2app::runtime::take_room_action(cx, room_id)
        {
            let RoomScreenWidgetRefs { user_profile_sliding_pane, .. } = self.cached_widget_refs(cx);
            self.apply_mini_app_room_action(cx, room_action, &user_profile_sliding_pane);
        }
    }
