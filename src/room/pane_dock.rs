...
pub fn set_pane_header(cx: &mut Cx, pane: &WidgetRef, kind: &RoomPaneKind) {
    ...
    // A mini-app's icon is a text glyph (e.g., an emoji), shown in place of the SVG icon.
    let glyph = match kind {
        RoomPaneKind::Members => {
            script_apply_eval!(cx, icon, { draw_icon +: { svg: (mod.widgets.ICON_MEMBERS) } });
            None
        }
        RoomPaneKind::PinnedMessages => {
...
impl RoomPaneDock {
    ...
    fn toggle(&mut self, cx: &mut Cx, kind: RoomPaneKind) {
        ...
        } else if self.create_pane(cx, kind, room_pane::last_layout(), None, None, true) {
            self.place_panes(cx, true);
        }
    }
    ...
    fn handle_request(&mut self, cx: &mut Cx, room_name_id: &RoomNameId, kind: &RoomPaneKind, op: RoomPaneOp) {
        match op {
            RoomPaneOp::Open => {
                if !self.has_pane(kind)
                    && self.create_pane(cx, kind.clone(), room_pane::last_layout(), None, None, true)
                {
                    self.place_panes(cx, true);
                }
            }
    ...
    fn move_pane(&mut self, cx: &mut Cx, room_name_id: &RoomNameId, kind: &RoomPaneKind, side: PaneSide, is_user_choice: bool) {
        ...
        if self.panes[index].layout.side == side {
            return;
        }
    ...
    fn layout_headers(&mut self, cx: &mut Cx) {
        ...
        for index in 0..self.panes.len() {
            ...
            let cols = if side.is_vertical() {
                ...
                let icon_width = if icon_width > 0.0 { icon_width } else { pane.glyph_width.unwrap_or(HEADER_ICON_WIDTH) };
...
pub enum RoomPaneEdgeAction {
    ...
    SplitMoved {
        side: PaneSide,
        /// An edge holds at most one pane of each kind, and usually just a few.
