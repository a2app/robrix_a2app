...
pub const CATALOG: &[Capability] = &[
    ...
    cap!("storage.quota.read", "Storage usage", "Bytes used and the cap, so an app can trim before writes fail.", Read, Outgoing, App, None, Available, Low, ["storage.quota"]),
    ...
    cap!("background.complete", "Finish a background run", "Acknowledges this instance's current background run. Cannot create, enable or reschedule tasks.", Act, Outgoing, Instance, None, Available, Low, ["background.complete"]),
    cap!("on_background", "Background task trigger", "A user-enabled scheduled, alarm, manual or room-message run. Room-message input keeps its room permission and information-flow checks.", Read, Incoming, Instance, None, Available, Low, ["on_background"]),
    ...
    cap!("events.subscribe", "Subscribe to an event family", "Turn on one incoming hook for this instance. Checked against that hook's own capability, records Used, and dies with the isolate.", Act, Outgoing, Instance, None, Available, Low, ["events.subscribe"]),
    cap!("events.unsubscribe", "Unsubscribe", "Stop one hook, or all with event:'*'.", Act, Outgoing, Instance, None, Available, Low, ["events.unsubscribe"]),
    cap!("ui.pane.read", "Its pane state", "Surface (dock/tab/modal/parked), dock side, content size, foreground (on screen).", Read, Outgoing, Instance, None, Available, Low, ["ui.pane.read"]),
    cap!("ui.pane.close", "Quit itself", "Tear down this instance, same as the Close button. Runs after the broker drain, never mid-dispatch.", Act, Outgoing, Instance, None, Available, Low, ["ui.pane.close"]),
    ...
    cap!("on_focus_changed", "Foreground changed", "Called when this instance becomes, or stops being, the pane the user is looking at.", Read, Incoming, Instance, None, Available, Low, ["on_focus_changed"]),
    cap!("on_visibility_changed", "Visibility changed", "visible / hidden-tab / offscreen-room; the cue to pause timers and network.", Read, Incoming, Instance, None, PlannedNewPlumbing, Low, []),
    cap!("on_surface_changed", "Moved between surfaces", "Called after a pop-out, return to room, or dock side change.", Read, Incoming, Instance, None, Available, Low, ["on_surface_changed"]),
    cap!("on_suspend", "Suspend", "Called when its tab is hidden or Robrix is backgrounded; the host pauses its timers after the call returns. One event-pass budget.", Read, Incoming, Instance, None, PlannedNewPlumbing, Low, []),
    ...
    cap!("ipc.apps.list", "Apps that accept messages", "Ids, names and running flag of installed apps that declare ipc. Gated by ipc because it leaks the installed-app list.", Read, Outgoing, Apps, Some(P::Ipc), Available, Medium, ["ipc.apps_list"]),
    // ----- matrix-room-read -----
    ...
    cap!("matrix.room.thread.read", "Thread replies", "Replies inside one thread, oldest first.", Read, Outgoing, Room, Some(P::MatrixRoomRead), Available, High, ["matrix.thread_replies"]),
    cap!("matrix.room.messages.paginate", "Older history", "One page further back than the recent window.", Read, Outgoing, Room, Some(P::MatrixRoomRead), Available, High, ["matrix.older_messages"]),
    cap!("matrix.room.event.read", "One message", "A single event by id with sender, body, timestamp, reactions and edit state.", Read, Outgoing, Room, Some(P::MatrixRoomRead), Available, High, ["matrix.event"]),
    ...
    cap!("matrix.room.unread.read", "Unread count", "Unread and mention counts and the marked-unread flag.", Read, Outgoing, Room, Some(P::MatrixRoomInfo), Available, Low, ["matrix.unread"]),
    cap!("matrix.room.power_levels.read", "Room permissions", "What you may do here: invite, kick, ban, redact, notify room.", Read, Outgoing, Room, Some(P::MatrixRoomInfo), Available, Low, ["matrix.power_levels"]),
    cap!("matrix.room.link.create", "Permalink", "A matrix.to or matrix: link to the attached room or one of its events.", Read, Outgoing, Room, Some(P::MatrixRoomInfo), Available, Low, ["matrix.permalink"]),
    cap!("matrix.room.successor.read", "Read the upgraded room", "Learn where an upgraded (tombstoned) room continued.", Read, Outgoing, Room, Some(P::MatrixRoomInfo), Available, Low, ["matrix.successor"]),
    cap!("on_room_info_changed", "Room details changed", "Renamed, new topic or avatar, encrypted, tagged, or upgraded; coalesced.", Read, Incoming, Room, Some(P::MatrixRoomInfo), Available, Low, ["on_room_info_changed"]),
    cap!("on_room_unread_changed", "Unread count changed", "Latest unread and mention counts, coalesced.", Read, Incoming, Room, Some(P::MatrixRoomInfo), Available, Low, ["on_room_unread_changed"]),
    cap!("on_room_pins_changed", "Pins changed", "The new pinned event ids (contents need matrix-room-read).", Read, Incoming, Room, Some(P::MatrixRoomInfo), Available, Low, ["on_room_pins_changed"]),
    // ----- matrix-profile -----
    ...
    // ----- device-info -----
    cap!("device.info.read", "Device facts", "Platform, OS family, locale, time zone, desktop view mode, UI zoom.", Read, Outgoing, Device, Some(P::DeviceInfo), Available, Low, ["device.info"]),
    ...
    // ----- matrix-room-watch -----
    cap!("on_room_message", "New messages", "New messages in the attached room as they arrive (media as {msgtype, body} without bytes), batched per event pass.", Read, Incoming, Room, Some(P::MatrixRoomWatch), Available, High, ["on_room_message"]),
    cap!("on_room_message_changed", "Message edited or removed", "An existing message was edited or redacted.", Read, Incoming, Room, Some(P::MatrixRoomWatch), Available, High, ["on_room_message_changed"]),
    cap!("on_room_reaction", "Reactions", "A reaction added or removed on any event.", Read, Incoming, Room, Some(P::MatrixRoomWatch), Available, Medium, ["on_room_reaction"]),
    ...
    cap!("on_room_typing", "Who is typing", "The current set of typing users, latest-only per pass; watched only while an instance asked.", Read, Incoming, Room, Some(P::MatrixRoomWatch), Available, Medium, ["on_room_typing"]),
    cap!("on_room_receipt", "Read receipts moved", "Someone's read position moved (own and others); silent while read receipts are hidden in Robrix's settings.", Read, Incoming, Room, Some(P::MatrixRoomWatch), Available, Medium, ["on_room_receipt"]),
    ...
    cap!("on_rooms_changed", "Room list changed", "Rooms joined, left, renamed, reordered, or unread counts changed; coalesced diff per pass.", Read, Incoming, MultiRoom, Some(P::MatrixRoomsList), Available, High, ["on_rooms_changed"]),
    cap!("on_invite_received", "Invite received", "A new room invite arrived; joining stays a user action unless matrix-membership is granted.", Read, Incoming, MultiRoom, Some(P::MatrixRoomsList), Available, Medium, ["on_invite_received"]),
    cap!("on_unread_totals_changed", "Unread totals changed", "Account-wide unread and mention totals without per-room detail.", Read, Incoming, MultiRoom, Some(P::MatrixRoomsList), Available, Low, ["on_unread_totals_changed"]),
    // ----- matrix-rooms-read -----
    ...
    cap!("ui.pane.set_side", "Choose dock side", "Move to top/bottom/left/right, like the pane's move button.", Act, Outgoing, Instance, Some(P::RobrixUi), Available, Low, ["ui.pane.set_side"]),
    cap!("ui.pane.break_out", "Pop out its pane", "Pop out into its own tab (desktop) or view (mobile), same as the pop-out button.", Act, Outgoing, Instance, Some(P::RobrixUi), Available, Low, ["ui.pane.break_out"]),
    cap!("ui.pane.minimize", "Minimize its pane", "Collapse its docked pane into a chip over its room's timeline, same as the minimize button; it keeps running.", Act, Outgoing, Instance, Some(P::RobrixUi), Available, Low, ["ui.pane.minimize"]),
    cap!("ui.pane.restore", "Restore its pane", "Dock itself in its room again while minimized or hidden, once the user is looking at that room, e.g., to show something new; room apps only, and never switches rooms or tabs.", Act, Outgoing, Instance, Some(P::RobrixUi), Available, Medium, ["ui.pane.restore"]),
    ...
    cap!("ui.pane.set_badge", "Badge its pane or tab", "A count or dot on its pane header and tab, the quiet alternative to a popup.", Act, Outgoing, Instance, Some(P::RobrixUi), PlannedNewPlumbing, Low, []),
    ...
    // ----- robrix-preferences -----
    cap!("host.prefs.read", "Display settings", "View mode, UI zoom, send-on-enter, thumbnail size, show_read_receipts; never the privacy prefs and never a setter.", Read, Outgoing, Client, Some(P::RobrixPreferences), Available, Low, ["host.prefs"]),
