...
pub const CATALOG: &[Capability] = &[
    ...
    cap!("ui.pane.read", "Its pane state", "Surface (dock/tab/modal/parked), dock side, content size, minimized, foreground (on screen).", Read, Outgoing, Instance, None, Available, Low, ["ui.pane.read"]),
    ...
    cap!("on_surface_changed", "Moved between surfaces", "Called after break-out, return-to-room, or a dock side change.", Read, Incoming, Instance, None, Available, Low, ["on_surface_changed"]),
    ...
    cap!("ui.pane.set_side", "Choose dock side", "Move to top/bottom/left/right, same as the CycleEdge button.", Act, Outgoing, Instance, Some(P::RobrixUi), Available, Low, ["ui.pane.set_side"]),
    cap!("ui.pane.minimize", "Minimize to chip", "Collapse into the chips row; an app may shrink itself but never un-minimize itself.", Act, Outgoing, Instance, Some(P::RobrixUi), Available, Low, ["ui.pane.minimize"]),
    cap!("ui.pane.break_out", "Break out to a tab", "Move to its own desktop dock tab; refused in mobile view.", Act, Outgoing, Instance, Some(P::RobrixUi), Available, Low, ["ui.pane.break_out"]),
